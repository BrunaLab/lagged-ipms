...
Deterministic projections of $\lambda$ for Continuous Forest were significantly greater than for Forest Fragments ($P[\theta \geq \theta_{obs}]$ = `r det_stats$p_val`, N = 1000 randomization). We also found found significant effects of IPM Type, Habitat, and their interaction on projections of $\lambda$ (F(df1, df2) = F-value, p = p-value, η² = effect size). Generally speaking, the effects of model choice on $\lambda$ were consistent across habitats - both the deterministic $\lambda$ and the average $\lambda$ from stochastic IPMs were higher for Continuous Forest than for Forest Fragments (@tbl-lambdas). However, model choice did influence the absolute value of projections within a habitat. Deterministic $\lambda$ was nearly identical to the average $\lambda$ from kernel-resampled IPMs, which was unexpected given that stochasticity is predicted to reduce population growth rates [@doakCorrectlyEstimatingHow2005a; @tuljapurkarManyGrowthRates2003; @metcalfStatisticalModellingAnnual2015]. However the average projection of $\lambda$ was 1.5-2% lower when including lagged effects, with 70-76% of $\lambda$ projections below deterministic $\lambda$. These IPMs also appear to be more accurate - the underlying vital rate models for IPMs with lagged effects all had the best fit to the survey data (dAIC = 0, @tbl-aic). <!--# No longer true if random effect = different smooth for each year -->

Including lagged effects in IPMs also resulted in less predictable projections of population structure, both with (@fig-pop-states a) and across habitats (@fig-pop-states b, @tbl-vartest). This variability is particularly notable in forest fragments, where we have previously shown lagged effects have very large impacts on growth and survival [@scottDelayedEffectsClimate2022]. The deterministic IPM for Continuous Forest projected slightly more of the smallest and largest plants than the one for Forest Fragments. In other words, populations in forest fragments had proportionately less recruitment and fewer individuals growing into the larger, reproductive size classes [@brunaDemographicEffectsHabitat2005]. This shift towards intermediately sized, pre-reproductive plants in forest fragments is even more dramatic when using the parameter-resampled stochastic IPM (@fig-pop-states b).\
