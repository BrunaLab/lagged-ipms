...
#' @param n_mesh number of meshpoints to use when discretizing log_size.  Passed
#'   to `ipmr::discretize_pop_vector()`.
#' @param n total individuals. The sum of `n_log_size` and `n_sdlg` will equal
#'   this.
#'
#' @return a list of two elements, `n_log_size` (length = n_mesh) and `n_sdlg`
#'   (length 1)
#'   
make_pop_vec <- function(data, n_mesh = 100, n = 1) {
  ...
  #use ipmr function to discretize log_size.  No padding because we are assuming
  #we've captured the entire range of plant sizes
  established_vec <-
    ipmr::discretize_pop_vector(log_size,
                                n_mesh = n_mesh,
                                pad_low = 1,
                                pad_high = 1)
  ...
  prop_established <-
    (established_vec$n_log_size * n_established) / n_total
